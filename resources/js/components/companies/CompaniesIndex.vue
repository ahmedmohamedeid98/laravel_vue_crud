...
<div class="flex place-content-end mb-4">
        <div class="px-4 py-2 text-white bg-indigo-500 hover:bg-indigo-700 cursor-pointer">
            ...
            <template v-for="item in companies" :key="item.id">
                <tr class="bg-white">
                    ...
                    <td class="px-6 py-4 text-sm text-center leading-5 text-gray-900 whitespace-no-wrap">
                        <router-link :to="{ name: 'companies.edit', params: { id: item.id } }" class="inline-flex items-center px-4 py-2 bg-indigo-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150 mr-2">Edit</router-link>
